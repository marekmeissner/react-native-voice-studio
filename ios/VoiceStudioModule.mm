...
- (void)onError:(NSError *)error {
  if (rejectBlock != nil) {
    rejectBlock([NSString stringWithFormat:@"%@", @(error.code)],
...
RCT_EXPORT_METHOD(startRecording : (RCTPromiseResolveBlock)
...
RCT_EXPORT_METHOD(stopRecording) { [moduleImpl stopRecordingSession]; }

RCT_EXPORT_METHOD(openSettings) { [moduleImpl openAppSettings]; }

- (std::shared_ptr<facebook::react::TurboModule>)getTurboModule:
    (const facebook::react::ObjCTurboModule::InitParams &)params {
  return std::make_shared<facebook::react::NativeVoiceStudioModuleSpecJSI>(
      params);
}
